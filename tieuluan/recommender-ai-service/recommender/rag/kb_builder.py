...
class KnowledgeBaseBuilder:
    ...
    def __init__(self):
        ...
        self.graph_store = GraphStore() # Khởi tạo Graph Store
    ...
    def add_product_documents(self, service_name: str):
        ...
        for product in products:
            ...
            # ---> BỔ SUNG: XÂY DỰNG GRAPH KNOWLEDGE BASE <---
            product_node = product.get('name', 'Unknown')
            # 1. Thêm Quan hệ Category (Phân loại)
            self.graph_store.add_triple(product_node, "BELONGS_TO_CATEGORY", service_name)
            
            # 2. Thêm Quan hệ Giá trị (Giá bán)
            if price:
                self.graph_store.add_triple(product_node, "HAS_PRICE", f"{price} VND")
                
            # 3. Thêm Quan hệ Xếp hạng
            rating = product.get('rating') or product.get('average_rating')
            if rating:
                self.graph_store.add_triple(product_node, "HAS_RATING", f"{rating} Star")
                
            # 4. Thêm Quan hệ Đặc tả Kỹ thuật
            if 'specs' in product or 'specifications' in product:
                ...
                if isinstance(specs, dict):
                    for key, value in list(specs.items())[:5]:
                        self.graph_store.add_triple(product_node, f"HAS_SPEC_{key.upper()}", value)
        
        print(f"✓ Added {len(products)} products from {service_name} (Vector + Graph)")
...
- Competitive prices with frequent promotions
        ...
        # ---> BỔ SUNG: LƯU GRAPH DATABASE VÀO FILE <---
        self.graph_store.save_graph()
        print("✓ Graph Database tri thức đã được xây dựng và lưu!")
