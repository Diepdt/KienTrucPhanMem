...
class RAGChatService:
    """Service xử lý chat sử dụng RAG Hybrid (Vector + Graph)"""
    ...
    def _retrieve_context(self, query: str, customer_id: int = None, top_k: int = 5) -> str:
        ...
        # 1. Vector Search (FAISS / Fallback)
        relevant_docs = self.vector_store.similarity_search(query_embedding, k=top_k)
        
        # 2. Graph Database Search (Thực thể & Quan hệ)
        graph_facts = self.graph_store.search_subgraph(query)
        
        # Format context
        context_parts = []
        if graph_facts:
            context_parts.append("--- SỰ KIỆN TỪ GRAPH DATABASE ---")
            for f in graph_facts:
                context_parts.append(f"- {f}")
            context_parts.append("-----------------------------")
            
        context_parts.append("--- DOCUMENTS TỪ VECTOR STORE ---")
...
"""
        ...
        try:
            ...
            error_msg = f"Đang có lỗi với hệ thống phản hồi API AI lúc này: {str(e)[:50]}..."
            print(f"Error calling Gemini API: {e}")
            
            # Khắc phục trải nghiệm bằng cách vẫn trả về AI Recommendations fallback
            try:
                fallback_recs = self.behavior_model.recommend(customer_id, top_k=3)
            except:
                fallback_recs = []

            return {
                'response': error_msg,
                'recommended_products': fallback_recs,
