...
def update_inventory_stock(items):
    try:
        ...
        for item in items:
            ...
            if product_id is None or not product_type:
                continue
        ...
        resp = http_requests.post(
            f"{django_settings.PRODUCT_SERVICE_URL}/api/products/update-stock/",
            json={'items': payload}, timeout=5)
        return resp.status_code == 200
    except Exception as e:
        logger.error(f"update_inventory_stock error: {e}")
    return False
...
def get_revenue_group_for_item(item):
    snapshot = item.product_snapshot or {}
    category_name = str(snapshot.get('category_name', '')).strip()
    if category_name:
    ...
    return product_type.title() or 'Unknown'
...
class CreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        for item in cart['items']:
            ...
            product_subtitle = item.get('product_subtitle') or ''
    ...
    def get(self, request):
        ...
        if start_date_raw or end_date_raw:
            ...
            for item, amount in line_items:
                group_name = get_revenue_group_for_item(item)
